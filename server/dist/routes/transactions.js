...
const router = Router();
function getBalanceChange(amount, type) {
...
|--------------------------------------------------------------------------
*/
function getTransactionId(param) {
...
router.post("/", authenticate, async (req, res) => {
    try {
        ...
        if (!type || !accountId || !categoryId) {
            return res.status(400).json({
                message: "Amount, type, accountId and categoryId are required",
            });
        }
        ...
        const transaction = await prisma.$transaction(async (tx) => {
            ...
            const balanceChange = getBalanceChange(numericAmount, type);
...
router.put("/:id", authenticate, async (req, res) => {
    try {
        ...
        const id = getTransactionId(req.params.id);
        if (!id) {
            return res.status(400).json({
        ...
        const updatedTransaction = await prisma.$transaction(async (tx) => {
            ...
            const oldBalanceChange = getBalanceChange(oldAmount, existingTransaction.type);
            await tx.account.update({
                where: {
             ...
             */
            const newBalanceChange = getBalanceChange(numericAmount, type);
            await tx.account.update({
                where: {
                    id: accountId,
                },
...
router.delete("/:id", authenticate, async (req, res) => {
    try {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            const balanceChange = getBalanceChange(amount, existingTransaction.type);
