    ...
    try {
        ...
        if (!amount || !month || !year || !categoryId) {
            return res.status(400).json({
                message: "Amount, month, year and categoryId are required",
            });
        }
        // 2. Validate amount
        ...
        if (!Number.isInteger(budgetMonth) ||
            budgetMonth < 1 ||
            budgetMonth > 12) {
            return res.status(400).json({
        ...
        if (!Number.isInteger(budgetYear) ||
            budgetYear < 2000) {
            return res.status(400).json({
...
router.get("/", authenticate, async (req, res) => {
    try {
        ...
        res.status(500).json({
            message: "Something went wrong while fetching budgets",
        });
    }
});
// Update a budget
